...
class SendToPostSimpleAction extends Action
{
    ...
    protected function setUp(): void
    {
        ...
        $this->action(function (Model $record) {
            try {
                ...
                // Success - redirect to PostSimple
                Notification::make()
                    ->title('Successfully sent to PostSimple!')
                    ->body('Redirecting to PostSimple...')
                    ->success()
                    ->send();

                // Redirect to PostSimple
                $redirectUrl = $this->postSimpleUrl . '?batch=' . $batchId;
                
                $this->redirect($redirectUrl, shouldOpenInNewTab: true);
